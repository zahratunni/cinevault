...
<div class="max-w-3xl mx-auto">
    ...
    <div class="bg-white rounded-lg shadow-md p-6 md:p-8">
        <form action="{{ route('admin.jadwals.update', $jadwal->jadwal_id) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
                ...
                <div class="bg-gray-100 px-4 py-3 rounded-lg border border-gray-300">
                    <div class="flex items-center space-x-3">
                        ...
                        <div>
                            ...
                            <p class="text-sm text-gray-600">{{ $jadwal->film->durasi_menit }} menit • {{ $jadwal->film->genre }}</p>
                        </div>
                    </div>
                </div>
                <input type="hidden" name="film_id" value="{{ $jadwal->film_id }}">
                <p class="text-xs text-gray-500 mt-2">
                    <i class="fas fa-info-circle"></i> Film tidak dapat diubah setelah jadwal dibuat. Hapus dan buat jadwal baru jika ingin mengganti film.
                </p>
            </div>
                ...
                <select name="studio_id" required
                    ...
                    @foreach($studios as $studio)
                        <option value="{{ $studio->studio_id }}" 
                            {{ old('studio_id', $jadwal->studio_id) == $studio->studio_id ? 'selected' : '' }}>
                ...
                <div class="flex items-start">
                    ...
                    <div>
                        ...
                        <p class="text-sm text-blue-700 mt-1">
                            Jam selesai akan dihitung otomatis: <strong id="jamSelesaiInfo">{{ \Carbon\Carbon::parse($jadwal->jam_selesai)->format('H:i') }}</strong>
                        </p>
            ...
            @if($jadwal->pemesanans()->whereIn('status_pemesanan', ['Lunas', 'Menunggu Bayar'])->count() > 0)
                ...
                <div class="flex items-start">
                    ...
                    <div>
                        ...
                        <p class="text-sm text-yellow-700 mt-1">
                            Jadwal ini memiliki <strong>{{ $jadwal->pemesanans()->whereIn('status_pemesanan', ['Lunas', 'Menunggu Bayar'])->count() }} pemesanan aktif</strong>.
                            Hati-hati saat mengubah waktu atau studio!
                        </p>
                    </div>
                </div>
            </div>
...
<script>
    ...
    const filmDurasi = {{ $jadwal->film->durasi_menit }}; // Get durasi from current film
    ...
    function calculateJamSelesai() {
        ...
        if (jamMulai) {
            ...
            const totalMinutes = parseInt(filmDurasi) + 15;
            ...
            jamSelesaiInfo.textContent = `${endHours}:${endMinutes} (${filmDurasi} menit film + 15 menit buffer)`;
        } else {
